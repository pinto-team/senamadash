...
import { useListProductsPage } from './useListProductsPage'
import { ProductsTableSkeleton, ProductsPaginationSkeleton } from './Skeletons'
import { ProductsEmpty } from './Empty'
import ProductsTable from '@/features/products/components/layout/Table/ProductsTable'
...
export default function ListProductsPage() {
    ...
    const content = () => {
        ...
        if (status.isLoading) return <ProductsTableSkeleton />
        if (list.items.length === 0) return <ProductsEmpty />

        return <ProductsTable items={list.items} onDelete={actions.handleDelete} />
    }
                    ...
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            ...
                            <Input
                                ...
                                aria-label={t('products.search_placeholder') as string}
                                className="w-72 [padding-inline-start:2rem]"
                            />
                        </div>

                        <Button onClick={() => nav.navigate(nav.ROUTES.PRODUCT.NEW)}>
                            {t('products.create')}
                        </Button>
                    </div>
                </div>
